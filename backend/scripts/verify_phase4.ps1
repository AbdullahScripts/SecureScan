# Phase 4 MalConv AI scanner verification
# Run from backend/:  .\scripts\verify_phase4.ps1
...
Write-Host "Phase 4 MalConv AI scanner verification"
...
if (-not (Test-Path -LiteralPath $samplePath)) {
    ...
    $stub = "MZ`r`nSafe Phase 4 test stub for MalConv/YARA/VT API verification only.`r`n"
...
# --- 1. Direct service test (predict function) ---
Write-Host "[1] Direct predict() call (Python in-process)"
Write-Step "Starting Python in-process check..."
$pyStep1 = @'
from app.services.ai_scanner_service import predict
import os

# Use test_sample.bin (we're already in backend directory)
test_file = os.path.join(os.getcwd(), 'test_sample.bin')
result = predict(test_file)
print('OK: predict() returned', result)
assert 'label' in result, 'Missing label in result'
assert 'confidence' in result, 'Missing confidence in result'
assert 'score' in result, 'Missing score in result'
assert result['label'] in ('benign', 'malicious', 'unavailable'), 'Unexpected label: %s' % result['label']
assert 0.0 <= result['confidence'] <= 1.0, 'Confidence out of range: %s' % result['confidence']
'@
& $pythonExe -c $pyStep1
...
# --- 2. HTTP scan test ---
Write-Host "[2] POST /scan/file (MalConv active)"
...
$email = "malconv_{0}_{1}@example.com" -f ([guid]::NewGuid().ToString("N").Substring(0, 12)), (Get-Random -Maximum 999999)
Write-Step "Signup: unique email = $email"

Write-Step "Before POST /auth/signup (building JSON body)..."
$signupBody = (@{ full_name = "MalConv Test"; email = $email; password = "securepass123" } | ConvertTo-Json -Compress)
...
Write-Step "OK: YARA still active"
Write-Host ""
